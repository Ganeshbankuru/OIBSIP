...
def button_press(entry_weight,feet_entry,inch_entry):
    if entry_weight.get() == 'null' or feet_entry.get() == 'null' or inch_entry.get() == 'null':
        error_dis()
        exit(0)
...
def error_dis():
    lbl_error = tk.Label(root,text="you have provided no data",font=("arial",12),bg='gold',fg='red')
...
lbl_title = tk.Label(root,text='Body Mass Index',font=('Arial',18),bg='gold')
...
lbl_weight = tk.Label(bmi_frame,text="Enter Weight (kg)",font=('Arial',12),bg='gold')
...
lbl_height = tk.Label(bmi_frame,text="Enter Height(feet&inch)",font=('Arial',12),bg='gold')
...
bmi_button = tk.Button(root,text="calculate",bg='lightblue',font=("roboto",12))
...
lbl_bmi = tk.Label(root,text=f"BMI:",bg='gold',font=("Arial",12))
lbl_bmi.pack(padx=10,pady=10)

#creating lable for showing the result of the bmi
lbl_res = tk.Label(root,text='Result',font=("roboto",14),bg="gold")
